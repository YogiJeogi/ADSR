...
SVD::SVD( MatDoub_I &a ) : m(a.nrows()), n(a.ncols()), u(a), v(n,n), w(n), pinv(n,m,0.)
{
...
void SVD::PeudoInverseMatrix()
{
	...
	for ( i=0 ; i < n ; i++ ){
		for ( j=0 ; j < m ; j++ ){
			for ( k=0 ; k < n ; k++ ){
				 if( k-1 > 0) { if(w[k-1]-w[k] >= 0.1) temp=0.; }
				else { temp = 1./w[k]; }
				pinv[i][j] += temp*v[i][k]*u[j][k];				
			}	
		}
	}
...
void SVD::SVDInitialization( MatDoub_I &a )
{
...
CPeudoInverse3D::CPeudoInverse3D(const Mat3DDoub_I &a) :n(a.dim1()), m(a.dim2()), k(a.dim3()), PINV3D(m,n,k) {
	...
	for( Index_K=0 ; Index_K < k ; Index_K++ ){
		...
		// 3D Matrix에서 k인덱스에 따른 매트릭스 추출
		MatDoub tMat(n,m,0.);
		for ( Index_I=0; Index_I < n; Index_I++){
			for ( Index_J=0; Index_J < m; Index_J++ ){
				tMat[Index_I][Index_J]=a[Index_I][Index_J][Index_K];
			}
		}

		//(pSVD+ik)->initialization(tMat); // 대입
		//(pSVD+ik)->peudoinverse();       // 연산
		//PINV3D.assignMatrixtokindex(ik,(pSVD+ik)->pinv);

		tSVD.SVDInitialization(tMat); // 대입
		
		tSVD.PeudoInverseMatrix();       // 연산

		PINV3D.AssignMatrixTOThirdIndex(Index_K,tSVD.pinv);
	}
	
}

Mat3DDoub & CPeudoInverse3D::GetPinverseMatrix3D()
{
	return PINV3D;
}

CPeudoInverse3D::~CPeudoInverse3D(){
