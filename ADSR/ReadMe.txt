...
ADSR.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CADSRApp 응용 프로그램
    클래스를 선언합니다.

ADSR.cpp
    응용 프로그램 클래스 CADSRApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
res\Toolbar.bmp
    ...
    ADSR.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음
    단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////
...
ADSRDoc.h, ADSRDoc.cpp - 문서
    이 파일에는 CADSRDoc 클래스가 들어 있습니다.  이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CADSRDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      asc
        파일 형식 ID:        ADSR.Document
        주 프레임 캡션:  ADSR
        문서 형식 이름:       ADSR
        필터 이름:         ADSR Files (*.asc)
        파일의 새 약식 이름: ADSR
        파일 형식의 긴 이름: ADSR.Document

ADSRView.h, ADSRView.cpp - 문서 뷰
    이 파일에는 CADSRView 클래스가 들어 있습니다.
    CADSRView 개체는 CADSRDoc 개체를 보는 데 사용됩니다.

res\ADSRDoc.ico
    CADSRDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다.  이 아이콘은 주 리소스 파일인 ADSR.rc에
    의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일인 ADSR.pch와
